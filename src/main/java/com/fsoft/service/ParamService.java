...
import com.fsoft.dao.UserRepository;
...
public class ParamService {
    ...
    public User checkDangNhap() {
        // check login
        if (cookieService.get("user") != null) {
            for (User i : nguoidungDAO.findAll()) {
                if (cookieService.getValue("user").equalsIgnoreCase(i.getUsername())) {
                    return i;
                }
            }
        } else if (sessionService.get("username") != null && cookieService.get("user") == null) {
            for (User i : nguoidungDAO.findAll()) {
                if (sessionService.get("username").equals(i.getUsername())) {
                    return i;
                }
            }
        }
    ...
    public String checkDangNhapString(String defaultAdmin, String defaultUser) {
        if (checkDangNhap() != null) {
            if (checkDangNhap().getLoginPermission()) {
                return defaultAdmin;
            } else if (!checkDangNhap().getLoginPermission()) {
                return defaultUser;
            }
        }
